<script lang="ts">
  ...
  let { modelName, setModelName, onClose, onDisconnect }:
    { modelName: string; setModelName: (m: string) => void; onClose: () => void; onDisconnect?: () => void } = $props();
...
<div onclick={onClose} style="position:fixed;inset:0;z-index:50;background:rgba(20,28,24,.4);backdrop-filter:blur(3px);display:grid;place-items:center;animation:fadein .2s ease both;padding:24px">
  <div onclick={(e) => e.stopPropagation()} class="card" style="width:560px;max-width:100%;padding:0;overflow:hidden;animation:pop .25s var(--spring) both">
    ...
    <div style="padding:24px">
      <div style="display:flex;align-items:center;gap:10px;padding:12px 13px;border-radius:var(--radius-sm);background:var(--surface-2);border:1.5px solid var(--border);margin-bottom:20px">
        ...
        <div style="min-width:0">
          ...
          <div style="font-size:11px;color:var(--faint);font-weight:600">Powers every AI feature in BoloDB</div>
        </div>
      </div>

      <div style="font-size:13px;font-weight:700;color:var(--ink-2);margin-bottom:8px">Model</div>
      <select class="field" bind:value={model} style="cursor:pointer;font-size:14px">
        {#each MODELS as m}
          <option value={m.id}>{m.label}</option>
        {/each}
      </select>
      <div style="font-size:12px;color:var(--faint);margin:7px 0 20px;font-weight:550">Not sure? Stay with "Balanced" — it works well for most databases.</div>

      <div style="font-size:13px;font-weight:700;color:var(--ink-2);margin-bottom:8px">Gemini API key</div>
