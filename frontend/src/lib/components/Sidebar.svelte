      ...
      <div class="rise" style="padding:2px 0 6px 27px">
        ...
        {:else}
          {#each history as h (h._id)}
            <div role="presentation" class="group" style="position:relative;margin-bottom:2px">
              ...
              <button onclick={(e) => { e.stopPropagation(); handleDelete(h._id); }} class="opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto focus-visible:opacity-100 focus-visible:pointer-events-auto focus:opacity-100 focus:pointer-events-auto transition-opacity" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:var(--surface);border:none;color:var(--faint);cursor:pointer;padding:4px;font-size:10px;border-radius:4px;display:flex;align-items:center;justify-content:center" title="Delete" aria-label="Delete '{h.question}' from history">✕</button>
            </div>
          {/each}
        {/if}
      </div>
