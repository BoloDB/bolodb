<script lang="ts">
  ...
  type Tab = "ask" | "dash" | "schedules" | "settings";
  ...
  const navItems: { label: string; icon: string; key: Tab }[] = [
    ...
    { label: "Schedules", icon: "⏱", key: "schedules" },
  ...
  async function finishRename(id: string) {
    ...
    if (
      title &&
      title !== (conversations.find((c) => c._id === id)?.title || "")
    ) {
      try {
        ...
        console.error(e);
        appState.showError(
          "Couldn't rename that conversation — please try again.",
        );
  ...
  /**
   * Ask and Settings are tabs *within* /chat; every other nav item is its own
   * route. So the question the handlers below actually ask is "are we on /chat
   * right now?", not "are we on dashboards?" — get that wrong and the buttons
   * that hand off through the URL become dead on any new route.
   */
  const ROUTED_TABS: Partial<Record<Tab, string>> = {
    dash: "/dashboards",
    schedules: "/schedules",
  };
  const onChatRoute = $derived(activeTab === "ask" || activeTab === "settings");

  // Selecting a destination on mobile should dismiss the drawer.
  function selectTab(t: Tab) {
    onClose?.();
    const route = ROUTED_TABS[t];
    if (route) {
      goto(route);
      return;
    }
    // Ask and Settings live on /chat. When another route is showing, switching
    // to them means navigating back — the shell stays mounted, so this reads as
    // a tab change rather than a page load.
    if (!onChatRoute) {
  ...
  /**
   * Routes that aren't /chat mount this sidebar without a conversation handler,
   * so a click there used to hit a no-op default and look like a dead button.
   * Hand off through the URL instead — /chat reads `?conversation=` on the way
   * in and opens it.
   */
  ...
  function wsTitle(ws: any): string {
    ...
    if (appState.activeWorkspace?.id === ws.id) {
      parts.push(
        appState.dbInfo ? "Database connected" : "No database connected",
      );
    }
    return parts.join(" · ");
  }
  ...
  <div class="modal-backdrop" onclick={() => (showCreateWsModal = false)}>
    ...
    >
      <div class="modal-header">
        ...
        <button class="close-btn" onclick={() => (showCreateWsModal = false)}
          >✕</button
        >
      </div>
      ...
      <div class="modal-footer" style="justify-content: flex-end;">
        <button
          class="btn btn-ghost"
          onclick={() => (showCreateWsModal = false)}>Cancel</button
        >
        <button
...
<div class="sidebar-wrapper" class:mobile-open={mobileOpen}>
  ...
  <div class="ws-rail">
    ...
    <div style="position:relative">
      <button
        class="ws-avatar ws-add"
        onclick={(e) => {
          e.stopPropagation();
          wsAddMenuOpen = !wsAddMenuOpen;
          wsMenuOpen = false;
        }}
        title="Add Workspace">+</button
      >
      {#if wsAddMenuOpen}
        <div
          class="ws-popover"
          onclick={(e) => e.stopPropagation()}
          style="bottom: 0; top: auto; left: 52px; transform: none;"
        >
          <button
            class="ws-p-item"
            onclick={() => {
              wsAddMenuOpen = false;
              showCreateWsModal = true;
  ...
  <aside class="sidebar">
    ...
    <div class="brand">
      ...
      <span class="brand-name"
        >Bolo<span style="color:var(--brand)">DB</span></span
      >
      <button
        class="mobile-close"
    ...
    <div class="panel-toggle">
      <div class="seg">
        <button
          ...
          onclick={() => (sidePanel = "chats")}>Chats</button
        >
        <button
          class:on={sidePanel === "schema"}
          onclick={() => (sidePanel = "schema")}>Schema</button
        >
      </div>
      <button
        class="new-chat"
      ...
      {#if sidePanel === "chats"}
            ...
            {#each conversations as cv (cv._id)}
              <div class="convo-row group">
                ...
                {:else}
                  <button
                    class="convo"
                    class:active={activeConversationId === cv._id}
                    onclick={() => selectConversation(cv._id)}
                  >
                    <div
                    ...
                    >
                      ...
                      {#if cv.database_id}
                        <span
                          class="convo-db"
                          title={conversationDbTitle(cv.database_id)}
                        >
                    ...
                    <span class="convo-meta"
                      >{cv.turn_count} turn{cv.turn_count === 1 ? "" : "s"} · {formatTime(
                        cv.updated_at,
                      )}</span
                    >
                  </button>
                  <button
                    class="convo-ren opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
                  ...
                  <button
                    class="convo-del opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
                    aria-label="Delete conversation"
                    title="Delete"
                    onclick={(e) => handleDelete(cv._id, e)}>✕</button
                  >
                {/if}
              </div>
            {/each}
            <button class="clear-all" onclick={handleClearConvs}
              >Clear all</button
            >
          </div>
        {:else if loadingConvs}
          <div
            style="display:flex;align-items:center;justify-content:center;padding:24px 0;color:var(--text-light);font-size:12px;gap:6px"
          >
            <svg
              width="14"
              height="14"
        ...
        {:else}
          <span class="empty-hint"
            >No chats yet.<br />Press + to start one.</span
          >
        {/if}
      {:else}
          ...
          {#each schemaData as t}
            <div>
              <button
                class="tbl"
                onclick={() =>
                  (openTable = openTable === t.name ? null : t.name)}
              >
                <span
                  ><span
                    class="chev"
                    style="transform:{openTable === t.name
                      ? 'rotate(90deg)'
                      : 'rotate(0deg)'}">›</span
                  >{t.name}</span
                >
    ...
    <div class="footer">
      <div class="trust" title={trust.behaviour}>
        ...
        <div class="trust-track">
          <div class="trust-fill" style="width:{trustPct}"></div>
        </div>
      </div>
