...
<div style="text-align:center">
  ...
  {/if}
  <div bind:this={containerEl} style="display:flex;justify-content:center;{loading ? 'opacity:0.6;pointer-events:none;' : ''}"></div>
  {#if !window.__GOOGLE_CLIENT_ID__ && !error}
    <button onclick={getClientId} style="display:inline-flex;align-items:center;gap:10px;padding:10px 24px;border:1px solid var(--border);border-radius:24px;background:var(--surface);color:var(--ink);font-size:14px;font-weight:500;cursor:pointer">
