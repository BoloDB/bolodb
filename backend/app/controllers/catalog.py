...
async def get_catalog(workspace_id, db, kb):
    ...
    return {"catalog": await kb.get_catalog(workspace_id, db.get_db_id(workspace_id))}


async def save_catalog(workspace_id, db, kb, payload):
    """Persist ``payload`` as the connected database's catalog."""
    if not db.connected(workspace_id):
        raise HTTPException(409, "No database connected")
    await kb.set_catalog(workspace_id, db.get_db_id(workspace_id), payload.model_dump())
    return {"ok": True}


async def suggest(workspace_id, db, providers):
    ...
    if not db.connected(workspace_id):
        raise HTTPException(409, "No database connected")
    base = suggest_from_schema(db.get_schema(workspace_id))
    try:
        enriched = await llm_suggest_catalog(
            providers.get(workspace_id), db.schema_as_text(workspace_id)
        )
    except Exception:
